...
export default function NewsletterForm({ variant = 'inline', source = 'footer' }) {
  ...
  if (status === 'success') {
    return (
      <div className={`rounded-xl border border-green-200 bg-green-50 p-5 ${variant === 'banner' ? 'mx-4 sm:mx-6 lg:mx-8 mt-8' : ''}`}>
        <div className="flex items-center gap-3">
          <CheckCircle size={20} className="text-green-600 shrink-0" />
          <div>
            <span className="text-sm font-bold text-green-700">¡Suscrito!</span>
            <p className="text-xs text-green-600 mt-0.5">Cada domingo recibirás "El Filtro" — el resumen semanal de Contexto Claro.</p>
          </div>
        </div>
      </div>
    )
